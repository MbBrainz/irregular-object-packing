...
class Optimizer(OptimizerData):
    ...
    def setup(self):
        ...
        if has_c_violations:
            raise ValueError(
                "Initial object placements show container violations for" f" {len(has_c_violations)} objects."
            )
        ...
        for i_b in range(0, self.settings.n_scaling_steps):
            ...
            for i in range(self.settings.itn_max):
                ...
                if np.alltrue([arr[0] >= self.scaling_barrier_list[i_b] for arr in self.tf_arrs]):
                    self.log(
                        ("All objects have reached the scaling barrier" f" {self.scaling_barrier_list[i_b]}"),
                        LOG_LVL_INFO,
                    )
                    break
            self.pbar1.update()
