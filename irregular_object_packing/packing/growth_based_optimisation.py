...
def constraint_multiple_points(x, facets, v):
    constr = []  # list of constraints
    for v_i in v:
        constr.append(constraint_single_point(x, facets, v_i))
...
facets = [
    np.array([[-4, -4, 1], [4, -4, 1], [4, 4, 1], [-4, 4, 1]], dtype=np.float64),
    np.array([[-4, -4, 0], [4, -4, 0], [4, 4, 0], [-4, 4, 0]], dtype=np.float64),
    np.array([[-4, -4, 1], [4, -4, 1], [-4, -4, 0], [4, -4, 0]], dtype=np.float64),
    np.array([[4, -4, 0], [4, -4, 1], [4, 4, 1], [4, 4, 0]], dtype=np.float64),
    np.array([[-4, -4, 1], [-4, -4, 0], [-4, 4, 0], [-4, 4, 1]], dtype=np.float64),
...
constraint_dict = {"type": "ineq", "fun": constraint_multiple_points, "args": (facets, v)}
res = minimize(objective, x0, method="SLSQP", bounds=bounds, constraints=constraint_dict)
# %%
# Print the results
print("Optimal solution:")
