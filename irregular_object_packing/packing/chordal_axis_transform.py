...
def compute_cat_faces_new(tetmesh: pv.UnstructuredGrid, point_sets, obj_coords: list[np.ndarray]) -> CatData:
    ...
    for cell in rel_cells:
        ...
        if cell.nobjs == 2:
            create_faces_2(data, occ, tetpoints)
        elif cell.nobjs == 3:
            create_faces_3(data, occ, tetpoints)
        elif cell.nobjs == 4:
            create_faces_4(data, tetpoints)
        else:
            raise ValueError("Cell has more than 4 objects")
...
def compute_cat_faces(
    ...
    for cell in range(tetmesh.n_cells):
        ...
        for t in tet_points:
            interesting_cells.add(t.tet_id)
    return data
