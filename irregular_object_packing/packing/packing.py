#%%
import trimesh
from irregular_object_packing.packing.chordal_axis_transform import compute_cat_cells, face_coord_to_points_and_faces
from irregular_object_packing.packing.initialize import create_packed_scene, pack_objects, save_image
from irregular_object_packing.tools.profile import pprofile, cprofile, lineprofile
...
for k, v in tqdm(cat_cells.items()):
    ...
    object_mesh.vertices = trimesh.transformations.transform_points(
        ...
        translation_matrix(np.array([0, 0, 0]), objects_coords[k]))
