...
class TetPoint:
    ...
    def __init__(self, point: np.ndarray, p_id, obj_id=-1, cell_id=-1):
        self.vertex = point
        self.p_id = p_id
        self.obj_id = obj_id
    ...
    def __add__(self, o: object) -> "TetPoint":
        return TetPoint(self.vertex + o.vertex, self.cell_id)
...
class CatData:
    ...
    def add_cat_face_to_point(self, point: TetPoint, face: list[int], normal: np.ndarray) -> None:
        self.cat_faces[point.obj_id][point.p_id].append(np.array(face))
    ...
    def get_faceless_points_for(self, obj_id: int) -> list[np.ndarray]:
        """Get the faces of the CAT that have not been filled."""
        empty_faces = []
        for point in self.cat_faces[obj_id].keys():
            if len(self.cat_faces[obj_id][point]) == 0:
                empty_faces.append(point)
        return empty_faces

    def create_cat_cells_from_cat_faces(self) -> 'CatData':
        """Get the cells of the CAT that have been filled."""
        data2 = copy(self)
        data2.cat_cells = {}
        for k, v in self.cat_faces.items():
            data2.cat_cells[k] = []
            for _p, faces in v.items():
                for face in faces:
                    data2.cat_cells[k].append(list(face))
        return data2
