...
class TestInitialize(unittest.TestCase):
    ...
    def test_init_coordinates_coords(self):
        ...
        self.assert_correct_coordinates(
            coords, coverage_rate, container_volume, mesh_volume, min_distances[0], descr="sphere"
        )

        coords, _ = init_coordinates(self.containers[1], self.shapes[1], coverage_rate, f_init)
        self.assert_correct_coordinates(
            coords, coverage_rate, container_volume, mesh_volume, min_distances[1], descr="box"
        )

        coords, _ = init_coordinates(self.containers[2], self.shapes[2], coverage_rate, f_init)
        self.assert_correct_coordinates(
            coords, coverage_rate, container_volume, mesh_volume, min_distances[2], descr="cylinder"
        )

    def assert_correct_coordinates(
        self, coords, coverage_rate, container_volume, mesh_volume, min_distance, descr
    ):
        # enough coordinates
        ...
        is_inside_list = pv.PolyData(coords).select_enclosed_points(self.containers[0])
        self.assertListEqual(
            is_inside_list, [1] * len(coords), msg=f"not all coords are inside the container ({descr})"
        )
        ...
        self.assertTrue(all(not_too_close_list), msg=f"not all coords are far enough apart ({descr})")
...
class TestCoordIsCorrect(unittest.TestCase):
    ...
    def test_coord_is_inside_but_too_close_to_container(self):
        ...
        self.assertFalse(
            coord_is_correct(close_container_coord, self.c_sphere, self.object_coords, self.min_distance),
            "coord should be too close to container",
        )
